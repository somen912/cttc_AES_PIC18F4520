...
void I2C_restart()
{
    SSPCON2bits.RSEN=1;  // repeat start condition
    while(!SSPIF);
    SSPIF=0;
}
...
{
    /* after writing 0xA0 it will return NACK because
     *  currently no slave is connected to the I2C bus */
    SSPBUF = byte;
    while(!SSPIF);
    SSPIF=0;
    return ACKSTAT;
...
{
    /* after reading it will return ACK because
     *  currently no slave is connected to the I2C bus */
